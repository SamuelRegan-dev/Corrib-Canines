// Demo 2 — frozen snapshot: header drop-in, hero slideshow, scroll reveals,
// contact form. Kept separate from jsDemo1.js so Demo 1 can keep changing.
...
// Drop the header in on load. The short delay lets the browser settle on the
// off-screen start position first, so the 1s ease actually runs instead of
// snapping. setTimeout rather than rAF so it still fires in a background tab.
function dropHeader() {
    setTimeout(() => header.classList.add('loaded'), 50);
}

if (document.readyState === 'complete') {
    dropHeader();
} else {
    window.addEventListener('load', dropHeader);
}
